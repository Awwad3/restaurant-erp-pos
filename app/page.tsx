...
function ModuleWorkspace({ section, onClose, onNotice }: { section: 'inventory' | 'accounting' | 'employees' | 'reports'; onClose: () => void; onNotice: (message: string) => void }) {
  const content = {
    inventory: { eyebrow: 'إدارة المخزون', title: 'المخزون والتوريد', description: 'تابع الأرصدة، التوريدات، والهدر من مكان واحد.', icon: Boxes, action: 'توريد جديد', columns: ['الصنف', 'الموقع', 'الرصيد', 'الحد الأدنى', 'الحالة'], rows: [['صدور دجاج طازج', 'المطبخ الرئيسي', '42 كجم', '20 كجم', 'متوفر'], ['زيت قلي نباتي', 'المستودع', '8 لتر', '12 لتر', 'منخفض'], ['جبنة شيدر', 'المطبخ الرئيسي', '16 كجم', '10 كجم', 'متوفر'], ['أكواب ورقية 12 أونصة', 'المستودع', '320 قطعة', '500 قطعة', 'منخفض']] },
  ...
  return <div className="pos-overlay"><div className="pos-modal module-modal"><div className="pos-header"><div className="module-title"><div className="module-icon"><Icon size={21} /></div><div><p className="eyebrow">{content.eyebrow}</p><h2>{content.title}</h2><p>{content.description}</p></div></div><button className="close-button" onClick={onClose} aria-label="إغلاق"><X size={20} /></button></div><div className="module-toolbar"><div className="module-stats"><span><b>{content.rows.length}</b> سجلات معروضة</span><span><b>اليوم</b> آخر مزامنة</span></div><button className="primary-button" onClick={() => onNotice(`${content.action} متاح في النسخة التالية`)}><Plus size={17} />{content.action}</button></div><div className="module-table-wrap"><table className="module-table"><thead><tr>{content.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{content.rows.map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={`${row[0]}-${cell}`}>{index === row.length - 1 ? <span className={`status-pill ${cell.includes('منخفض') || cell.includes('متأخر') || cell.includes('إجازة') ? 'warning' : 'success'}`}>{cell}</span> : cell}</td>)}</tr>)}</tbody></table></div>{section === 'accounting' && <div className="accounting-summary"><div><small>إجمالي المدين</small><b>42,680 ر.س</b></div><div><small>إجمالي الدائن</small><b>42,680 ر.س</b></div><div><small>قيود غير مرحلة</small><b>3 قيود</b></div></div>}</div></div>
}
