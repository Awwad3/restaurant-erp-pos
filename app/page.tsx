    ...
    <main className="erp-shell" dir="rtl">
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label="التنقل الرئيسي">
          ...
          {navItems.map(({ label, icon: Icon, active, badge }) => <button key={label} className={`nav-item ${active ? 'active' : ''}`} onClick={() => { if (label === 'نقطة البيع') { setShowPOS(true); setShowOrders(false) } else if (label === 'الطلبات') { setShowOrders(true); setShowPOS(false) } else { flash(`${label} قيد التطوير`) } }}><Icon size={18} /><span>{label}</span>{badge && <em>{badge}</em>}</button>)}
