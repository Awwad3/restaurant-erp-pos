...
export default function Page() {
  ...
  const [activeSection, setActiveSection] = useState<'inventory' | 'accounting' | 'employees' | 'reports' | null>(null)
    ...
    <main className="erp-shell" dir="rtl">
      <aside className="sidebar">
        ...
        <nav className="main-nav" aria-label="التنقل الرئيسي">
          ...
          {navItems.map(({ label, icon: Icon, active, badge }) => <button key={label} className={`nav-item ${active ? 'active' : ''}`} onClick={() => { if (label === 'نقطة البيع') { setShowPOS(true); setShowOrders(false); setActiveSection(null) } else if (label === 'الطلبات') { setShowOrders(true); setShowPOS(false); setActiveSection(null) } else if (label === 'المخزون') { setShowPOS(false); setShowOrders(false); setActiveSection('inventory') } else if (label === 'المحاسبة') { setShowPOS(false); setShowOrders(false); setActiveSection('accounting') } else if (label === 'الموظفون') { setShowPOS(false); setShowOrders(false); setActiveSection('employees') } else if (label === 'التقارير') { setShowPOS(false); setShowOrders(false); setActiveSection('reports') } else { flash(`${label} قيد التطوير`) } }}><Icon size={18} /><span>{label}</span>{badge && <em>{badge}</em>}</button>)}
      ...
      <section className="workspace">
        ...
        <div className="content">
          ...
          <div className="lower-grid"><section className="panel kitchen-panel"><div className="panel-heading"><div><h2>حالة الم��بخ <span className="live-label"><i /> مباشر</span></h2><p>متابعة الطلبات في الوقت الحقيقي</p></div><button className="text-button" onClick={() => flash('عرض شاشة المطبخ الكاملة')}>عرض الكل <ArrowUpRight size={15} /></button></div><div className="ticket-list">{tickets.map((ticket) => <div className="ticket" key={ticket.id}><div className={`ticket-status ${ticket.status}`}><CookingPot size={17} /></div><div className="ticket-main"><div><b>{ticket.id}</b><span>{ticket.table}</span></div><p>{ticket.items}</p></div><div className="ticket-time"><Clock3 size={14} />{ticket.time}</div><button className="ticket-action" onClick={() => advanceTicket(ticket.id)}>{ticket.status === 'received' ? 'بدء التحضير' : ticket.status === 'preparing' ? 'جاهز' : 'تم التقديم'}</button></div>)}{tickets.length === 0 && <div className="empty-state"><Check size={20} />لا توجد طلبات متأخرة</div>}</div></section><section className="panel branch-panel"><div className="panel-heading"><div><h2>أداء الفرع</h2><p>مقارنة هذا الأسبوع</p></div><button className="icon-button"><MoreHorizontal size={18} /></button></div><div className="branch-score"><strong>92<span>%</span></strong><div><b>ممتاز</b><p>أعلى من الأسبوع الماضي</p></div></div><div className="progress-row"><div><span>المبيعات</span><b>86%</b></div><div className="progress"><i style={{ width: '86%' }} /></div></div><div className="progress-row"><div><span>رضا العملاء</span><b>94%</b></div><div className="progress"><i style={{ width: '94%' }} /></div></div><div className="progress-row"><div><span>كفاءة المطبخ</span><b>89%</b></div><div className="progress"><i style={{ width: '89%' }} /></div></div><div className="branch-meta"><span><Table2 size={15} /> 18 طاولة</span><span><UsersRound size={15} /> 12 موظفاً</span></div></section></div>
        </div>
      </section>

      {activeSection && <ModuleWorkspace section={activeSection} onClose={() => setActiveSection(null)} onNotice={flash} />}

      {showOrders && <div className="pos-overlay"><div className="pos-modal"><div className="pos-header"><div><p className="eyebrow">إدارة الطلبات</p><h2>الطلبات النشطة</h2></div><button className="close-button" onClick={() => setShowOrders(false)} aria-label="إغلاق"><X size={20} /></button></div><div className="orders-list">{[{ id: 'ORD-2084', type: 'صالة', table: 'طاولة 08', amount: '128 ر.س', status: 'قيد التحضير' }, { id: 'ORD-2083', type: 'تيك أواي', table: 'استلام من الفرع', amount: '86 ر.س', status: 'جاهز للاستلام' }, { id: 'ORD-2082', type: 'توصيل', table: 'شارع الملك فهد', amount: '154 ر.س', status: 'جديد' }].map((order) => <div className="order-row" key={order.id}><div><b>{order.id}</b><span>{order.type} · {order.table}</span></div><strong>{order.amount}</strong><em>{order.status}</em><button className="text-button" onClick={() => flash(`تم فتح ${order.id}`)}>عرض</button></div>)}</div></div></div>}
...
function ModuleWorkspace({ section, onClose, onNotice }: { section: 'inventory' | 'accounting' | 'employees' | 'reports'; onClose: () => void; onNotice: (message: string) => void }) {
  const [inventoryTab, setInventoryTab] = useState<'stock' | 'issues' | 'receipts'>('stock')
  const [showInventoryForm, setShowInventoryForm] = useState(false)
  const [formType, setFormType] = useState<'issue' | 'receipt'>('issue')
  const inventoryRows = [
    ['صدور دجاج طازج', 'المطبخ الرئيسي', '42 كجم', '20 كجم', 'متوفر'],
    ['زيت قلي نباتي', 'المستودع', '8 لتر', '12 لتر', 'منخفض'],
    ['جبنة شيدر', 'المطبخ الرئيسي', '16 كجم', '10 كجم', 'متوفر'],
    ['أكواب ورقية 12 أونصة', 'المستودع', '320 قطعة', '500 قطعة', 'منخفض'],
  ]
  const data = {
    accounting: { eyebrow: 'المحاسبة', title: 'شجرة الحسابات والقيود', description: 'إدارة الحسابات والقوائم المالية والقيود اليومية.', icon: ReceiptText, action: 'قيد جديد', columns: ['رقم الحساب', 'اسم الحساب', 'النوع', 'الرصيد', 'الحالة'], rows: [['1.1.1', 'الصندوق الرئيسي', 'أصول متداولة', '24,850 ر.س', 'نشط'], ['4.1', 'مبيعات الصالة', 'إيرادات', '8,420 ر.س', 'نشط'], ['5.1', 'تكلفة البضاعة المباعة', 'مصروفات', '2,180 ر.س', 'نشط'], ['2.1', 'الموردون', 'التزامات', '6,320 ر.س', 'نشط']] },
    employees: { eyebrow: 'الموارد البشرية', title: 'الموظفون والحضور', description: 'إدارة الفريق، الورديات، والحضور والانصراف.', icon: UsersRound, action: 'إضافة موظف', columns: ['الموظف', 'الدور', 'الوردية', 'الحضور', 'الحالة'], rows: [['محمد العتيبي', 'مدير الفرع', '08:00 - 17:00', 'حاضر', 'نشط'], ['سارة القحطاني', 'كاشير', '09:00 - 18:00', 'حاضر', 'نشط'], ['خالد المطيري', 'شيف', '14:00 - 23:00', 'متأخر 12 د', 'نشط'], ['نورة الحربي', 'ويتر', '16:00 - 00:00', 'لم يسجل', 'في إجازة']] },
    reports: { eyebrow: 'التقارير والتحليلات', title: 'التقارير المالية والتشغيلية', description: 'رؤى واضحة تساعدك على اتخاذ قرارات أفضل.', icon: BarChart3, action: 'تصدير التقرير', columns: ['التقرير', 'الفترة', 'آخر تحديث', 'النتيجة', 'الإجراء'], rows: [['تقرير المبيعات اليومية', 'اليوم', 'منذ 5 دقائق', '8,420 ر.س', 'عرض'], ['قائمة الدخل', 'هذا الشهر', 'اليوم 09:00 ص', 'صافي 18,240 ر.س', 'عرض'], ['حركة المخزون', 'هذا الأسبوع', 'اليوم 08:40 ص', '126 حركة', 'عرض'], ['تقرير الحضور', 'هذا الشهر', 'أمس 06:00 م', '96.4%', 'عرض']] },
  }[section]
  if (section === 'inventory') return <div className="pos-overlay"><div className="pos-modal module-modal inventory-modal"><div className="pos-header"><div className="module-title"><div className="module-icon"><Boxes size={21} /></div><div><p className="eyebrow">إدارة المخزون</p><h2>المخزون والتوريد</h2><p>الأرصدة، أوامر الصرف، والتوريد المخزني حسب مركز التكلفة.</p></div></div><button className="close-button" onClick={onClose} aria-label="إغلاق"><X size={20} /></button></div><div className="inventory-stats"><div><span>قيمة المخزون</span><b>48,620 ر.س</b></div><div><span>أصناف منخفضة</span><b className="warning-text">7 أصناف</b></div><div><span>أوامر قيد التنفيذ</span><b>5 أوامر</b></div><div><span>آخر مزامنة</span><b>اليوم 10:42 ص</b></div></div><div className="inventory-toolbar"><div className="inventory-tabs">{[['stock','الأرصدة الحالية'],['issues','أوامر الصرف'],['receipts','التوريد المخزني']].map(([key, label]) => <button key={key} className={inventoryTab === key ? 'selected' : ''} onClick={() => setInventoryTab(key as typeof inventoryTab)}>{label}</button>)}</div><div className="inventory-actions"><button className="secondary-button" onClick={() => { setFormType('issue'); setShowInventoryForm(true) }}><ArrowDownLeft size={16} />أمر صرف</button><button className="primary-button" onClick={() => { setFormType('receipt'); setShowInventoryForm(true) }}><Truck size={16} />توريد مخزني</button></div></div>{inventoryTab === 'stock' && <div className="module-table-wrap"><table className="module-table"><thead><tr>{['الصنف','الموقع','الرصيد','الحد الأدنى','الحالة'].map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{inventoryRows.map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={`${row[0]}-${cell}`}>{index === row.length - 1 ? <span className={`status-pill ${cell === 'منخفض' ? 'warning' : 'success'}`}>{cell}</span> : cell}</td>)}</tr>)}</tbody></table></div>}{inventoryTab === 'issues' && <InventoryOrders type="issue" onNotice={onNotice} />}{inventoryTab === 'receipts' && <InventoryOrders type="receipt" onNotice={onNotice} />}{showInventoryForm && <InventoryForm type={formType} onClose={() => setShowInventoryForm(false)} onSave={(message) => { setShowInventoryForm(false); onNotice(message) }} />}</div></div>
  const Icon = data.icon
  return <div className="pos-overlay"><div className="pos-modal module-modal"><div className="pos-header"><div className="module-title"><div className="module-icon"><Icon size={21} /></div><div><p className="eyebrow">{data.eyebrow}</p><h2>{data.title}</h2><p>{data.description}</p></div></div><button className="close-button" onClick={onClose} aria-label="إغلاق"><X size={20} /></button></div><div className="module-toolbar"><div className="module-stats"><span><b>{data.rows.length}</b> سجلات معروضة</span><span><b>اليوم</b> آخر مزامنة</span></div><button className="primary-button" onClick={() => onNotice(`${data.action} متاح في النسخة التالية`)}><Plus size={17} />{data.action}</button></div><div className="module-table-wrap"><table className="module-table"><thead><tr>{data.columns.map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{data.rows.map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={`${row[0]}-${cell}`}>{index === row.length - 1 ? <span className="status-pill success">{cell}</span> : cell}</td>)}</tr>)}</tbody></table></div>{section === 'accounting' && <div className="accounting-summary"><div><small>إجمالي المدين</small><b>42,680 ر.س</b></div><div><small>إجمالي الدائن</small><b>42,680 ر.س</b></div><div><small>قيود غير مرحلة</small><b>3 قيود</b></div></div>}</div></div>
}

function InventoryOrders({ type, onNotice }: { type: 'issue' | 'receipt'; onNotice: (message: string) => void }) {
  const rows = type === 'issue' ? [['ISS-0048', 'مركز تكلفة الإنتاج', 'صدور دجاج، زيت قلي', '18.5 كجم', 'معتمد'], ['ISS-0047', 'المطبخ الرئيسي', 'جبنة شيدر، صلصة', '12.0 كجم', 'قيد المراجعة'], ['ISS-0046', 'مركز تكلفة المشروبات', 'أكواب ورقية، حليب', '320 قطعة', 'مصروف']] : [['GRN-0092', 'شركة المذاق للتوريد', 'مواد غذائية متنوعة', '24,850 ر.س', 'مستلم'], ['GRN-0091', 'مورد التعبئة الحديثة', 'أكواب وعلب تيك أواي', '6,420 ر.س', 'قيد الاستلام'], ['GRN-0090', 'مؤسسة الطازج', 'خضروات ودجاج', '13,680 ر.س', 'مستلم']]
  return <div className="module-table-wrap"><table className="module-table"><thead><tr>{(type === 'issue' ? ['رقم الأمر','مركز التكلفة','الأصناف','الكمية','الحالة'] : ['رقم التوريد','المورد','الأصناف','القيمة','الحالة']).map((column) => <th key={column}>{column}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={`${row[0]}-${cell}`}>{index === row.length - 1 ? <span className={`status-pill ${cell.includes('مراجعة') || cell.includes('استلام') ? 'warning' : 'success'}`}>{cell}</span> : cell}</td>)}</tr>)}</tbody></table><button className="text-button" onClick={() => onNotice(type === 'issue' ? 'تم تحديث أوامر الصرف' : 'تم تحديث التوريدات')}>تحديث القائمة</button></div>
}

function InventoryForm({ type, onClose, onSave }: { type: 'issue' | 'receipt'; onClose: () => void; onSave: (message: string) => void }) {
  return <div className="form-overlay"><div className="inventory-form"><div className="pos-header"><div><p className="eyebrow">{type === 'issue' ? 'أمر صرف مخزني' : 'أمر توريد مخزني'}</p><h2>{type === 'issue' ? 'صرف مواد للإنتاج' : 'تسجيل توريد جديد'}</h2></div><button className="close-button" onClick={onClose} aria-label="إغلاق"><X size={20} /></button></div><div className="form-grid"><label>{type === 'issue' ? 'مركز التكلفة' : 'المورد'}<select defaultValue=""><option value="" disabled>اختر {type === 'issue' ? 'مركز التكلفة' : 'المورد'}</option><option>مركز تكلفة الإنتاج</option><option>المطبخ الرئيسي</option><option>مركز تكلفة المشروبات</option></select></label><label>الموقع المخزني<select defaultValue=""><option value="" disabled>اختر الموقع</option><option>المستودع الرئيسي</option><option>المطبخ الرئيسي</option></select></label><label>الصنف<select defaultValue=""><option value="" disabled>اختر الصنف</option><option>صدور دجاج طازج</option><option>زيت قلي نباتي</option><option>جبنة شيدر</option><option>أكواب ورقية</option></select></label><label>الكمية<input type="number" placeholder="0.00" /></label>{type === 'receipt' && <label>التكلفة للوحدة<input type="number" placeholder="0.00 ر.س" /></label>}<label className="form-wide">ملاحظات<textarea placeholder="أضف ملاحظات الأمر..." /></label></div><div className="form-footer"><button className="secondary-button" onClick={onClose}>إلغاء</button><button className="primary-button" onClick={() => onSave(type === 'issue' ? 'تم إنشاء أمر الصرف وإرساله لمركز تكلفة الإنتاج' : 'تم تسجيل أمر التوريد المخزني بنجاح')}><Check size={16} />حفظ الأمر</button></div></div></div>
}
